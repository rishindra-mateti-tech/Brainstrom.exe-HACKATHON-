...
export const analyzeIngredients = (
    ...
    if (userProfile?.skin_type === 'dry') {
        const fattyAlcohols = ['cetearyl', 'cetyl', 'stearyl', 'behenyl', 'arachidyl', 'myristyl', 'lanolin'];
        const hasDryingAlcohol = ingredientList.some(i =>
            i.includes('alcohol') && !fattyAlcohols.some(fa => i.includes(fa))
        );
        if (hasDryingAlcohol) {
            score -= 15;
