...
This version started as a university Machine Learning course project: a real ingredient-function classifier (TF-IDF + RandomForest) trained on the EU's public CosIng database, replacing the static lookup table used in v1. Multiple baseline models were compared, and the final pipeline uses a Class Sparsity Reduction Strategy to handle imbalanced data. (The course required submitting as a group of at least three; two teammates joined and put together the presentation deck. All modeling and engineering, in this file and the rest of the repository, is solo work.)
...
Built and maintained by **Rishindra Mateti**.
