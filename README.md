...
**Latest fixes (Rishindra Mateti)**: this version had sat untouched for months, and going back through it turned up a fake OAuth setup, an admin panel secured by nothing more than a hardcoded password, and an ML safety check that was silently failing. Fixed the auth and admin security, tracked down and fixed the ML bugs, retrained the classifier so it would actually fit on a free host, added a scrape-and-AI fallback for ingredients the model doesn't know (clearly labeled, never passed off as verified), and got the app properly deployed and working end to end again.
